...
public interface Blobs {
	 ...
	 * @param timestamp the timestamp of the blob
	 * @param verifier the verifier of the blob
	 * 
	 ...
	 */
	Result<Void> upload(String blobId, long timestamp, String verifier, byte[] bytes);
	 ...
	 *		   NOT_FOUND, if no blob matches the provided blobId
	 */
	default Result<Void> downloadToSink(String blobId, long timestamp, String verifier, Consumer<byte[]> sink) {
		var res = download(blobId, timestamp, verifier);
		if (!res.isOK())
